...
if ! command -v wg-quick >/dev/null 2>&1; then
    bashio::exit.nok 'WireGuard tools not found. Please ensure wireguard-tools package is installed.'
fi

if ! command -v wg >/dev/null 2>&1; then
    bashio::exit.nok 'WireGuard command not found. Please ensure wireguard-tools package is installed.'
fi
...
if ! grep -q "^\[Interface\]" "${wireguard_config}"; then
    bashio::exit.nok "Invalid WireGuard configuration: missing [Interface] section in ${wireguard_config##*/}"
fi

if ! grep -q "^\[Peer\]" "${wireguard_config}"; then
    bashio::exit.nok "Invalid WireGuard configuration: missing [Peer] section in ${wireguard_config##*/}"
fi

if ! grep -q "^PrivateKey" "${wireguard_config}"; then
    bashio::exit.nok "Invalid WireGuard configuration: missing PrivateKey in ${wireguard_config##*/}"
fi

bashio::log.info "WireGuard configuration validation passed."
